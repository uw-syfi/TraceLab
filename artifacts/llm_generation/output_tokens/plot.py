...
def plot_output_tokens(stats: dict[str, MetricStats], output_dir: Path, max_groups: int) -> None:
    ...
    ax.set_xlabel("Output Tokens (binary scale)")
    ax.set_ylabel("Invocations")
    ax.yaxis.set_major_formatter(mticker.StrMethodFormatter("{x:,.0f}"))
    ax.grid(True, alpha=0.3)
    apply_binary_token_axis(ax, axis="x", max_value=max_value, first_tick=first_tick, max_ticks=14)

    for index, (label, s) in enumerate(plotted):
        pcts = s.percentiles((50, 90))
        ax.hist(
            ...
            label=(
                f"{short_label(label)} "
                f"(n={s.count:,}, p50={pcts[50]:.0f}, p90={pcts[90]:.0f})"
            ),
        )
...
def main() -> int:
    ...
    plot_output_tokens(stats, out, args.max_groups)
